...
-- assumes quad pt faces; should/can we make it more generic?
local facesFromLines = function(lines)
                ...
                for d = 1, #lines, 1 do
                    if a ~= b and a ~= c and a ~= d and b ~= c and b ~= d and c ~= d then
                        ...
                        then
                            ...
                            if not isBad then table.insert(possible, { a, b, c, d }) end
                        end
                    end
                end
            end
    ...
    for _, v in ipairs(possible) do
        local count, isDup = 0, false
        local a, b, c, d = table.unpack(v)
        while not isDup and count < #faces do
            count = count + 1
            local knownFace = faces[count]
            isDup = isDup or (
        ...
        if not isDup then table.insert(faces, v) end
    end
    return faces
end
...
--         pLineMapping = pLineMapping, lFaceMapping = lFaceMapping,
