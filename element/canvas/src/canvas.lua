...
elementMT.__index = function(_, k)
    ...
    elseif obj.key then
        ...
        -- handle util._properties wrapper
        if k == "_properties" then return self end
...
moduleMT.rotateElement = function(self, index, angle, point, append)
    ...
    local initialTransform = append and self[index].transformation or uitk.util.matrix.identity()
...
module._legacy.compositeTypes   = module.compositeTypes
module._legacy.windowBehaviors  = uitk.window.behaviors
module._legacy.windowLevels     = uitk.window.levels

module._legacy.defaultTextStyle = module.defaultTextStyle
module._legacy.elementSpec      = module.elementSpec
module._legacy.help             = module.help
